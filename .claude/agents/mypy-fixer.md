...
Fixes mypy type errors and eliminates `Any` usage in the gmail-archiver project.
...
1. Fix all mypy errors reported by `uv run mypy gmail_archiver`
...
Place new TypedDict classes in `gmail_archiver/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`gmail_archiver/typing.py` when shared.
...
1. Run `uv run mypy gmail_archiver` and capture all errors.
...
4. Run `uv run mypy gmail_archiver` again to verify fixes.
