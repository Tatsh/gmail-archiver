...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `gmail_archiver/commands/` changed.**
...
For example, if a commit contains `gmail_archiver/commands/main.py`,
...
For Python files, strip the `gmail_archiver/` prefix and replace `/` with `.` (like module imports).

- Python file `gmail_archiver/media.py` → `media:`.
- Multiple files under `gmail_archiver/commands/` → `commands:`.
- Single command file `gmail_archiver/commands/admin.py` → `commands.admin:`.
